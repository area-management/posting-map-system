...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.data === 'json') {
     return ContentService.createTextOutput(JSON.stringify(getMapData())).setMimeType(ContentService.MimeType.JSON);
  }
  // 通常はHTMLを表示
...
function getAppData() {
  ...
  if (guideSheet) {
    ...
    if (typeof totalDistributed === 'string') {
      totalDistributed = parseInt(totalDistributed.split('/')[0].replace(/,/g, '')) || 0;
    }
  }

  const exclude = [CONFIG.SHEET_GUIDE, CONFIG.SHEET_ROSTER, CONFIG.SHEET_TEMPLATE, CONFIG.SHEET_POSTAL, CONFIG.SHEET_DISTRICT, CONFIG.SHEET_MASTER_EXPORT, CONFIG.SHEET_REPORT, CONFIG.SHEET_MANUAL];
  const areas = ss.getSheets()
 ...
 * モバイルアプリ用：配布報告
 */
function submitDistribution(areaName, rowId, staffName, count, isDone) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(areaName);
  ...
  sheet.getRange(rowId, 4).setValue(isDone);
  sheet.getRange(rowId, 5).setValue(isDone ? now : "");
  sheet.getRange(rowId, 6).setValue(count);
  sheet.getRange(rowId, 7).setValue(staffName);
...
function getCsvOrSheetData(filename) {
  ...
  if (mime === MimeType.GOOGLE_SHEETS) {
    ...
    try { return Utilities.parseCsv(text); } catch (e) { return text.split('\n').map(line => line.split(',')); }
  }
}
...
function toHalfWidth(str) {
  return str.toString().replace(/[０-９]/g, s => String.fromCharCode(s.charCodeAt(0) - 0xFEE0));
}
...
function generateAreaSheetsBatch() {
  ...
  for (let currentIndex = startIndex; currentIndex < addresses.length; currentIndex++) {
    ...
    if (now - startTime > 300 * 1000) { // 5分制限
      props.setProperty('BATCH_INDEX', currentIndex.toString());
      // プロパティへの保存はBATCH_INDEXのみでOK（シミュレーションでcityCountsは復元可能）
      ScriptApp.newTrigger('generateAreaSheetsBatch').timeBased().after(1000 * 60).create();
    ...
    let sheetName = cityCounts[currentCity] === 1 ? currentCity : `${currentCity}(${cityCounts[currentCity]})`;
    ...
    const displayAddress = currentAddr.postalCode ? `〒${currentAddr.postalCode}\n${currentAddr.address}` : currentAddr.address;
  ...
  ss.toast("すべてのエリアシートの展開（市町村境界考慮・10件分割版）が完了しました！", "完了", 10);
...
function aggregateTotalVolumes() {
  ...
  const exclude = [targetSheetName, CONFIG.SHEET_ROSTER, CONFIG.SHEET_TEMPLATE, CONFIG.SHEET_POSTAL, CONFIG.SHEET_DISTRICT];
  ...
  sheets.forEach(sheet => {
    ...
    if (!exclude.includes(name) && !sheet.isSheetHidden()) {
      const total = parseFloat(sheet.getRange("H1").getValue()) || 0;
      grandTotal += total;
      const lastRow = sheet.getLastRow();
      if (lastRow >= 2) {
        const data = sheet.getRange(2, 6, lastRow - 1, 2).getValues();
        data.forEach(row => {
          if (row[1] && typeof row[0] === 'number' && row[0] > 0) {
            staffCounts[row[1]] = (staffCounts[row[1]] || 0) + row[0];
          }
        });
      }
    }
  ...
  if (destSheet) {
    ...
    destSheet.getRange("H5:K5").merge().setValue(`${grandTotal.toLocaleString()} / ${goal.toLocaleString()}`);
    ss.toast(`配布総数は ${grandTotal.toLocaleString()} 枚です。`, "更新完了");
  }
}

function getRoster() {
  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_ROSTER);
    if (!sheet) return [];
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) return [];
...
function showAppUrl() {
  ...
  SpreadsheetApp.getUi().showModalDialog(HtmlService.createHtmlOutput(html).setWidth(400).setHeight(200), 'スマホアプリURL');
}
...
function refreshAreaSummaryCache() {
  ...
  // キャッシュに保存（文字列化して保存）
  PropertiesService.getScriptProperties().setProperty('AREA_SUMMARY_CACHE', JSON.stringify(result));
...
function onOpen() {
  ...
  ui.createMenu('⚙️ ポスティング管理')
    ...
    .addSeparator()
    .addItem('🔍 ドライブのファイルを確認する (レスキュー)', 'diagnoseDriveFiles')
    .addSeparator()
    .addItem('⚡ アプリ起動を高速化（キャッシュ更新）', 'refreshAreaSummaryCache')
    .addSeparator()
    .addItem('🎨 全シートを「プロ仕様」に一斉整形', 'formatAllSheets')
    .addToUi();
}

function extractCityName(addr) { return addr.match(/^(.+?[市郡])/) ? addr.match(/^(.+?[市郡])/)[1] : "エリア"; }
function deleteTriggers(name) { ScriptApp.getProjectTriggers().forEach(t => { if (t.getHandlerFunction() === name) ScriptApp.deleteTrigger(t); }); }
...
function diagnoseDriveFiles() {
  ...
  try {
    ...
    while (files.hasNext()) {
      ...
      if (name.toLowerCase().endsWith('.csv') || file.getMimeType() === MimeType.GOOGLE_SHEETS) {
        msg += `・${name}\n`;
        count++;
      }
...
function updateSheetSummary(sheet) {
  ...
  let total = 0;
  data.forEach(row => { if (row[0] === true && typeof row[2] === 'number') total += row[2]; });
...
function deleteAllAreaSheets() {
  ...
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const exclude = [CONFIG.SHEET_GUIDE, CONFIG.SHEET_ROSTER, CONFIG.SHEET_TEMPLATE, CONFIG.SHEET_POSTAL, CONFIG.SHEET_DISTRICT];
  ss.getSheets().forEach(s => { if (!exclude.includes(s.getName())) ss.deleteSheet(s); });
  ss.toast("リセット完了しました。");
}

function createManualSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(CONFIG.SHEET_MANUAL) || ss.insertSheet(CONFIG.SHEET_MANUAL);
  sheet.clear();
  sheet.getRange("B2").setValue("ポスティング報告 らくらくガイド").setFontSize(24).setFontWeight("bold");
  ss.toast("マニュアル作成完了。");
}

function exportAllDataToMasterSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let master = ss.getSheetByName(CONFIG.SHEET_MASTER_EXPORT) || ss.insertSheet(CONFIG.SHEET_MASTER_EXPORT);
  master.clear();
  ss.toast("マスター抽出完了。");
}
