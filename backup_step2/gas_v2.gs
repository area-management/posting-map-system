...
function aggregateTotalVolumes() {
  ...
  sheets.forEach(sheet => {
    ...
    if (!exclude.includes(name) && !sheet.isSheetHidden()) {
      const lastRow = sheet.getLastRow();
      if (lastRow >= 2) {
        // A:住所, B:地図, C:メモ, D:完了, E:日時, F:枚数, G:スタッフ
  ...
  if (guideSheet) {
    ...
    guideSheet.getRange("H5:K5").merge().setValue(
      `全体進捗: ${progressPercent.toFixed(1)}%`
    );
    
    // 2. 総配布枚数表示 (H6セル)
    guideSheet.getRange("H6:K6").merge().setValue(
      `総配布枚数: ${grandTotalVolume.toLocaleString()} 枚`
    );
